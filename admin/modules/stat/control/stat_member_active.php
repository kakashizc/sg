...
class stat_member_activeControl extends SystemControl{
    private $links = array(
        //array('url'=>'act=stat_member_active','lang'=>'stat_active_member'),
    );
    ...
    public function newmemberOp(){
        ...
        if($this->search_arr['search_type'] == 'day'){
            ...
            $field .= ' ,DAY(FROM_UNIXTIME(time)) as dayval,HOUR(FROM_UNIXTIME(time)) as hourval ';
            if (C('dbdriver') == 'mysql' || C('dbdriver') == 'mysqli' || C('dbdriver') == 'mysqli') {
                $_group = 'dayval,hourval';
            } elseif (C('dbdriver') == 'oracle') {
                $_group = "DAY(FROM_UNIXTIME(time)),HOUR(FROM_UNIXTIME(time))";
            }
        ...
        if($this->search_arr['search_type'] == 'week'){
            ...
            $where['time'] = array('between', array($stime,$etime));
            $field .= ',WEEKOFYEAR(FROM_UNIXTIME(time)) as weekval,WEEKDAY(FROM_UNIXTIME(time))+1 as dayofweekval ';
            if (C('dbdriver') == 'mysql' || C('dbdriver') == 'mysqli' || C('dbdriver') == 'mysqli') {
                $_group = 'weekval,dayofweekval';
            } elseif (C('dbdriver') == 'oracle') {
                $_group = 'WEEKOFYEAR(FROM_UNIXTIME(time)),WEEKDAY(FROM_UNIXTIME(time))+1';
            }
            $memberlist = $model->statByMember($where, $field, 0, '', $_group);
        ...
        if($this->search_arr['search_type'] == 'month'){
            ...
            $where['time'] = array('between', array($stime,$etime));
            $field .= ',MONTH(FROM_UNIXTIME(time)) as monthval,day(FROM_UNIXTIME(time)) as dayval ';
            if (C('dbdriver') == 'mysql' || C('dbdriver') == 'mysqli') {
                $_group = 'monthval,dayval';
            } else if (C('dbdriver') == 'oracle') {
                $_group = 'MONTH(FROM_UNIXTIME(time)),day(FROM_UNIXTIME(time))';
            }
            $memberlist = $model->statByMember($where, $field, 0, '', $_group);
            if($memberlist){
                foreach($memberlist as $k => $v){
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $excel_obj->addWorksheet($excel_obj->charset('激活会员统计',CHARSET));
            $excel_obj->generateXML($excel_obj->charset('激活会员统计',CHARSET).date('Y-m-d-H',time()));
            exit();
        } else {
            //得到统计图数据
            $stat_arr['title'] = '激活会员统计';
            $stat_arr['yAxis'] = '激活会员数';
            $stat_json = getStatData_LineLabels($stat_arr);
            Tpl::output('stat_json',$stat_json);
            Tpl::output('statlist',$statlist);
            Tpl::output('count_arr',$count_arr);
            Tpl::output('top_link',$this->sublink($this->links, 'newmember'));
			Tpl::setDirquna('stat');/*网 店 运 维shop wwi.com*/
            Tpl::showpage('stat.active_member');
        }
    }
    /**
     * 会员分析
    ...
    public function showmemberOp(){
        ...
        if (!empty($_GET['t'])){
            ...
            $where['time'] = array('between',$searchtime_arr);
        }
        if (!empty($_GET['id']) && is_array($_GET['id'])){
        ...
        if (is_array($member_list)){
            foreach ($member_list as $k=> $v){
                $member_list[$k]['time'] = $v['time']?date('Y-m-d H:i:s',$v['time']):'';
        ...
        if ($this->search_arr['exporttype'] == 'excel'){
            ...
            foreach ($member_list as $k => $v){
                ...
                $excel_data[$k+1][] = array('data'=>$v['time']);
            ...
            $excel_data = $excel_obj->charset($excel_data,CHARSET);
            $excel_obj->addArray($excel_data);
            $excel_obj->addWorksheet($excel_obj->charset('激活会员',CHARSET));
            $excel_obj->generateXML($excel_obj->charset('激活会员',CHARSET).date('Y-m-d-H',time()));
            exit();
        }
    ...
    public function get_member_xmlOp(){
        ...
        if (!empty($list) && is_array($list)){
			$fields_array = array('*');
            foreach ($list as $k => $v){
                //$out_array = getFlexigridArray(array(),$fields_array,$v,array());
				$out_array = array();
                $out_array['username'] = $v['username'];
				 $out_array['num'] = $v['num'];
                $out_array['time'] = date('Y-m-d H:i:s',$v['time']);
                $out_array['name'] = $v['name'];
                $out_array['tel'] = $v['tel'];
				$out_array['qq'] = $v['qq'];
				$out_array['email'] = $v['email'];
				$out_array['status'] = ($v['status']==1)? '<span class="yes"><i class="fa fa-check-circle"></i>是</span>' : '<span class="no"><i class="fa fa-ban"></i>否</span>';;
                $statlist[$v['id']] = $out_array;
            }
        }
        $data = array();
        $data['now_page'] = $model->shownowpage();
