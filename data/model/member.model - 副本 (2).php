...
class memberModel extends Model
{
    ...
    public function doActive($net_id, $userid, $biz_id, $lsk, $rid)
    {
        $record = Model('record');
        $net = Model('net');
        ...
        try {
            ...
            $r = $this->table("jiang")->insert($data);
            if (!$r) {
                $res['msg'] = '写入奖金记录失败';
                throw new Exception();
            }
    ...
    public function insertRegToday($id,$l_num=0,$r_num=0){
        ...
        if($regtoday){
            ...
            $this->table("reg_day")->where("id=".$regtoday['id'])->update($data);
            return ;
        }else {
            ...
            $data["l_num"] = $l_num;
            $data["r_num"] = $r_num;
            $this->table("reg_day")->insert($data);
        }
            ...
            if ($fman = $this->net_model->getNetByUser($us['pid'])) {
                switch ($treeplace) {
                    case 1:
                        ...
                        $this->insertRegToday($fman['uid'],1,0);

//                        if($star){
//                            $num_star = unserialize($fman['l_num_star']);
//                            $num_star[$star] = $num_star[$star]+1;
//                            $fman_update['l_num_star'] = serialize($num_star);
//
//                        }
                        break;
                    case 2:
                        ...
                        $this->insertRegToday($fman['uid'],0,1);

//                        if($star){
//                            $num_star = unserialize($fman['r_num_star']);
//                            $num_star[$star] = $num_star[$star]+1;
//                            $fman_update['r_num_star'] = serialize($num_star);
//                        }
              ...
              //  $star = $this->updateSatrLevel($userInfo, $userNet);
                //升级为报单中心
                $this->updateBiz($userInfo, $userNet);
